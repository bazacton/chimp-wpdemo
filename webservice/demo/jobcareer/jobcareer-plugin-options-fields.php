if (!class_exists('jobcareer_options_fields')) {

    class jobcareer_options_fields {
	 ...
	 */
	public function jobcareer_cs_fields($cs_setting_options) {
		...
		switch ($value['type']) {
			...
			$cs_opt_array = array(
			    'name' => $value['name'],
			    'counter' => $cs_counter,
                            'title2' => esc_html('De-register Purchase Code', 'jobcareer'),
                            'title2class' => 'jobcareer-dereg-code-area jobcareer-dereg-btn',
			    'id' => $value['id'],
			...
			break;
		    case "profilecheck_package":
			$obj = new ProfileCheck_Package_Fields();
			$output .= $obj->cs_profile_check_package_section();
			break;
		    case "cv_pkgs":
			$obj = new cs_plugin_options();
			...
			if ($cs_post_loc_country != '') {
			    $selected_spec = get_term_by('slug', $selected_country, 'cs_locations');
                            $state_parent_id = array();
                            if(isset($selected_spec) && $selected_spec != ''){
